...
namespace myNP
{
	...
	CS_FIRE_PACKET CS_FIRE_PACKET::MakePacket(uint32_t bullet_id, float x, float y, bool direction, uint32_t bullet_type, std::chrono::microseconds fire_time)
	{
		...
		return CS_FIRE_PACKET(bullet_id, x, y, direction, bullet_type, fire_time);
	}
	...
	SC_FIRE_PACKET SC_FIRE_PACKET::MakePacket(uint32_t bullet_id, float x, float y, bool direction, uint32_t bullet_type, std::chrono::microseconds fire_time)
	{
		...
		return SC_FIRE_PACKET(bullet_id, x, y, direction, bullet_type, fire_time);
	}
	...
	SC_BULLET_REMOVE_PACKET SC_BULLET_REMOVE_PACKET::MakePacket(uint32_t player_id, uint32_t bullet_id)
	{
		...
		return SC_BULLET_REMOVE_PACKET(player_id, bullet_id);
	}
	...
	SC_ITEM_CREATE_PACKET SC_ITEM_CREATE_PACKET::MakePacket(uint32_t item_id, float x, float y)
	{
		...
		return SC_ITEM_CREATE_PACKET(item_id, x, y);
	}
	...
	SC_ITEM_REMOVE_PACKET SC_ITEM_REMOVE_PACKET::MakePacket(uint32_t item_id)
	{
		...
		return SC_ITEM_REMOVE_PACKET(item_id);
	}
	...
	SC_GUN_UPDATE_PACKET SC_GUN_UPDATE_PACKET::MakePacket(uint32_t player_id, uint32_t gun_id)
	{
		...
		return SC_GUN_UPDATE_PACKET(player_id, gun_id);
	}
	...
	SC_LIFE_UPDATE_PACKET SC_LIFE_UPDATE_PACKET::MakePacket(uint32_t player_id)
	{
		...
		return SC_LIFE_UPDATE_PACKET(player_id);
	}
